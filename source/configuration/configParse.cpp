				...
				if (key == "methods") {
					...
					for (size_t i = 1; i < w.size(); ++i) {
						...
						if (m != "GET" && m != "POST" && m != "DELETE")
							throw std::runtime_error("config: invalid method '" + m + "' (only GET, POST, DELETE are allowed)");
						srv.ser_methods.push_back(m);
					}
					continue;
				}
				if (key == "client_max_body_size") {
					if (w.size() != 2) throw std::runtime_error("config: expects one argument");
					srv.client_max_body_size = w[1];
					continue;
				}
				if (key == "alias") {
